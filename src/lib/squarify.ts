/**
 * Squarified treemap layout (ported from sm64ds-decomp/tools/treemap.py).
 * Pure, deterministic, matches the Python version used for the README SVG.
 *
 * items: array of {value: positive number, ...rest}
...
export function squarify<T extends { value: number }>(
  ...
  let scaled = filtered.map(it => ({ it, area: it.value * scale }))
