...
import { Search, ChevronRight, ChevronDown, X, Link2, FileCode, Lock, RefreshCw, Plus, Minus, Palette, Settings, MessageCircle, User, Github } from 'lucide-react'
...
// Only a project supplied by the URL should override a saved one. Deliberately not
// keyed on DATA_URL_INIT: that also resolves out of localStorage, so gating the saved
// config on it would make a saved project discard its own identity and fall back to
// whatever this build happens to have bundled.
const HAS_URL_LINK = !!(LINK_REPO || _param('data'))
...
// Sibling decomps offered in the header switcher. Deliberately only identity and
// where the data lives: compiler, verify command, rules and the rest are read from
// whatever each project publishes in its own chaos-db, so this list never carries a
// second copy that can drift from the project it describes.
const SIBLINGS: { name: string; title: string; github: string; dataUrl: string }[] = [
  {
    name: 'sm64ds-decomp',
  ...
  {
    name: 'pictochat-decomp',
    title: 'PictoChat',
    github: 'https://github.com/tangOS-decomps/pictochat-decomp',
    dataUrl: 'https://raw.githubusercontent.com/tangOS-decomps/pictochat-decomp/chaos-data/chaos-db.json',
  },
]

const AUTHOR = 'Brennen (Tango)'
const AUTHOR_URL = 'https://github.com/tangosdev'
...
const EXAMPLES: { label: string; data: string }[] = SIBLINGS.map(s => ({ label: s.name, data: s.dataUrl }))
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(bustNow(dataUrl)).then(r => r.ok ? r.json() : Promise.reject(r.status)).then((j: ChaosDb) => {
      if (cancelled) return
      // Layered, not replaced: a project whose published block is thin (name/title
  ...
  // Switching project invalidates every module-level value derived from the data URL
  // (DETAILS_BASE, P, the share link, the details cache), so persist the choice and
  // reload instead of patching each one back into agreement. Dropping the query string
  // is the part that matters: a ?data= or ?repo= link outranks the saved project, so
  // carrying it across would silently undo the switch on the very next load.
  const switchProject = useCallback((name: string) => {
    const sib = SIBLINGS.find(s => s.name === name)
    if (!sib || sib.name === P.name) return
    localStorage.setItem('chaos-project', JSON.stringify({ name: sib.name, github: sib.github, dataUrl: sib.dataUrl }))
    location.href = location.origin + location.pathname
  }, [])

  // Atlas cosmetics from the project's backend: the RESOLVED contributor color map. The
  // backend is authoritative, we just render what it sends, so a color bought in the shop
  // looks the same here, in the Console and on the site.
  //
  // Derived from claimsApi with NO FALLBACK on purpose: a backend serves exactly one
  // project, so defaulting a project that has none to somebody else's base would paint the
  ...
  // only refined once the project's data lands.
  useEffect(() => { if (P.name) document.title = `Chaos Viewer — ${P.name}` }, [db])
  ...
  async function claimFunctions(fns: ChaosFunction[]) {
    ...
    if (!claimSession) {
      if (P.claimsAuthUrl) signInWithGitHub()
  ...
  const contributors = useMemo(() => {
    ...
    // The backend resolves the authoritative color per contributor (colors bought in the
    // shop, plus de-conflicted defaults so nobody shares a shade); use it directly so this
    // atlas matches the Console's and the site's. The palette is only a fallback for a
    // contributor the backend has not ranked yet. Logins are compared lowercased because
    // the backend stores them as typed.
    const resolved = new Map(Object.entries(cosmetics?.colors ?? {}).map(([login, hex]) => [login.toLowerCase(), hex]))
    return ranked.map(([handle, c], i) => ({
      handle, ...c,
      color: resolved.get(handle.toLowerCase()) ?? CONTRIB_PALETTE[i % CONTRIB_PALETTE.length],
    }))
  }, [db, cosmetics])
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      const items: { fn: ChaosFunction; det: FunctionDetail | null }[] = []
      ...
      <div className="relative z-10 w-full px-3 sm:px-4 py-4 select-none">
        <header className="mb-4 flex flex-col lg:flex-row gap-3 lg:justify-between items-start select-none">
          <div className="aero-banner px-6 py-4 flex flex-col items-center justify-center text-center">
            ...
            {P.github ? (
              // Blue-on-blue at 14% alpha made this nearly invisible against the glass
              // banner, so the pill sits on white and the label keeps full opacity.
              <div className="mt-2 inline-flex items-center gap-1 text-[12px] font-medium rounded-full pl-2.5 pr-1 py-1"
                   style={{ background: 'rgb(255 255 255 / 0.75)', border: '1px solid rgb(var(--aero-primary-rgb) / 0.55)',
                            boxShadow: 'inset 0 1px 0 rgb(255 255 255 / 0.9), 0 1px 3px rgb(var(--aero-primary-rgb) / 0.2)' }}>
                <span className="uppercase tracking-wide text-[10px] font-semibold" style={{ color: 'var(--aero-primary)' }}>Project</span>
                <span className="relative inline-flex items-center">
                  <select value={P.name} onChange={e => switchProject(e.target.value)}
                          className="mono appearance-none bg-transparent border-0 outline-none cursor-pointer text-aero-text text-[12px] font-medium pl-1 pr-4"
                          title="switch project">
                    {SIBLINGS.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
                    {!SIBLINGS.some(s => s.name === P.name) && <option value={P.name}>{P.name}</option>}
                  </select>
                  <ChevronDown className="w-3 h-3 absolute right-0.5 pointer-events-none" style={{ color: 'var(--aero-primary)' }} />
                </span>
                <a href={P.github} target="_blank" rel="noreferrer" title={`open ${P.name} on GitHub`}
                   className="inline-flex items-center rounded-full p-1 text-aero-muted hover:text-aero-primary">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="w-full lg:w-72 xl:w-80 flex-shrink-0 win overflow-hidden flex flex-col" style={{ minHeight: '420px', maxHeight: '80vh' }}>
            ...
            <div className="px-2 pb-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-aero-muted border-b border-white/70 mb-2">
        ...
        <footer className="mt-10 pointer-events-auto text-center pb-4 flex justify-center">
          ...
          <div className="aero-panel px-6 py-3 space-y-2 max-w-[760px]">
            ...
            <div className="text-[11px] text-aero-muted">
              Special thanks to{' '}
              <a href="https://github.com/lunavyqo" target="_blank" rel="noreferrer" className="text-aero-primary underline hover:text-aero-text">Lunavyqo</a>
              {' '}and{' '}
              <a href="https://github.com/andrewboudreau" target="_blank" rel="noreferrer" className="text-aero-primary underline hover:text-aero-text">Andrew</a>
